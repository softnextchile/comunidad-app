  ...
  <div>
    <AdminLayout>
      <div class="page-header">
        ...
        <button class="btn-primary" @click="openCreate">+ Nuevo Evento</button>
      </div>
      ...
      <div class="table-card">
        <table class="table">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              ...
              <td class="td-date">{{ formatDate(item.fecha) }}</td>
              <td>{{ item.lugar || '—' }}</td>
        ...
        <div v-if="items.length === 0" class="empty">No hay eventos</div>
      </div>
      <!-- Pagination -->
        ...
        <div class="modal">
          ...
          <div class="modal-body">
            <div class="form-group">
              <label>Título *</label>
              <input v-model="form.titulo" type="text" placeholder="Nombre del evento" />
            </div>
            <div class="form-group">
              <label>Descripción *</label>
              <textarea v-model="form.descripcion" placeholder="Descripción del evento" rows="3" />
            </div>
            <div class="form-row-3">
              ...
              <div class="form-group">
                ...
                <input v-model="form.capMax" type="number" min="0" placeholder="Sin límite" />
              </div>
            </div>
            <div class="form-row-2">
              <div class="form-group">
                <label>Lugar</label>
                <input v-model="form.lugar" type="text" placeholder="Sede Social, etc." />
              </div>
              <div class="form-group">
      ...
      <div v-if="showDelete" class="modal-overlay" @click.self="showDelete = false">
        <div class="modal modal-sm">
          <div class="modal-header"><h3>Eliminar Evento</h3></div>
          <div class="modal-body">
            <p>¿Eliminar "<strong>{{ deleting?.titulo }}</strong>"?</p>
          </div>
          <div class="modal-footer">
...
const defaultForm = () => ({ titulo: '', descripcion: '', fecha: '', hora: '', lugar: '', categoria: 'REUNION', capMax: '', publicado: false, id: null as number | null })
const form = reactive(defaultForm())

async function load() {
  const data = await $fetch<any>(`/api/admin/eventos?page=${currentPage.value}`)
...
async function save() {
  ...
  try {
    ...
    if (editing.value) {
      await $fetch(`/api/admin/eventos/${form.id}`, { method: 'PUT', body: payload })
    } else {
      await $fetch('/api/admin/eventos', { method: 'POST', body: payload })
    }
...
async function doDelete() {
  saving.value = true
  try {
    await $fetch(`/api/admin/eventos/${deleting.value.id}`, { method: 'DELETE' })
